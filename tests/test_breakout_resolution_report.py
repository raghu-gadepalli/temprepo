#!/usr/bin/env python3
"""Unified BREAKOUT_RESOLUTION signal report (read-only, offline).

Edit TEST SETTINGS and run:

    python tests/test_breakout_resolution_report.py

Purpose
-------
Generate a compact hypothetical signal report from the same immutable snapshots
used by the existing AutoTrades replay.  The program does not write signals,
trades, setup state, audit rows, or snapshots.  It treats ACCEPTED_BREAKOUT and
FAILED_BREAKOUT as alternate resolutions of the same exact structural-boundary
interaction and emits each unique episode resolution once for analytical comparison.

The implementation reads only neutral snapshot facts. It owns its structural
level discovery, boundary episode state, price-action interpretation, acceptance
policy, failure policy, and arbitration. It does not consume the current
ACCEPTED_BREAKOUT or FAILED_BREAKOUT status/CREATE definitions.

Candidate replacement policy
----------------------------
* DYNAMIC_BOUNDARIES_ONLY controls the event universe. It defaults to True, so
  only current INTRADAY_BALANCE dynamic-range highs/lows create episodes.
* When the flag is False, current dynamic accepted-range boundaries have
  priority over ORB/PDH/PDL.
* A fixed ORB/PD level is stale when accepted dynamic value has fully migrated
  through it; the same stale-level rule is used for acceptance and failure.
* Acceptance requires the locally defined completed-close count for the exact
  level class, price-action confirmation, HMA alignment, participation,
  dynamic-range age, structural room, acceptable entry distance, and no
  terminal-extension condition.
* Failure is derived from the episode chronology: a genuine outside auction,
  meaningful re-entry, two completed inside closes, post-reclaim directional
  follow-through, opposite-side price action, acceptable entry distance, and
  sufficient remaining room to the opposite frozen range edge. The old total-
  range-width gate is not used.
* A fresh accelerating trend against the failure entry is blocked unless the
  move is already extreme. Bollinger/RSI stretch protections remain conservative.
* One persistent boundary episode owns both acceptance and failure evidence.
  Episode identity is structural and does not contain a moving attempt timestamp. A same-boundary sequence can restart only after two
  completed closes materially back inside its frozen value.
  Outcome policies remain distinct, but they are evaluated from the same frozen
  event context and lifecycle.
* Consumed/terminal resolutions are removed before stock-level arbitration, so
  an old winning candidate cannot suppress a newer valid candidate.
* The exact event-specific accepted range is frozen from the historical snapshot
  associated with the boundary attempt whenever possible.
* Signal quality is measured independently of TradeManager. The compact exit is
  a fixed nine-subsequent-candle comparison mark (or end of day when fewer bars
  remain), with 3/5/9-candle and full-session MFE/MAE also reported.
* A hypothetical stock-level Advisor is observation-only. At each signal it uses
  only the stock snapshots available up to that time to classify the intraday
  regime and return ALLOW, DEFER, or COUNTERTREND_EXCEPTION. It never changes
  or suppresses the generated signal.

Outputs
-------
* breakout_resolution_report.csv
* test_breakout_resolution_report.log

The first columns remain the requested compact signal report. Diagnostic fields
follow them so signal-definition differences can be explained without treating
this analytical exit as production trade management.
"""
...
OUTPUT_CSV_PATH: str = "breakout_resolution_report.csv"
LOG_FILE: str = "test_breakout_resolution_report.log"
...
POLICY_VERSION: str = (
    "BR_V3_SELF_CONTAINED_DYNAMIC_ONLY_STOCK_ADVISOR"
    if DYNAMIC_BOUNDARIES_ONLY and ENABLE_HYPOTHETICAL_STOCK_ADVISOR
    else "BR_V3_SELF_CONTAINED_DYNAMIC_ONLY"
    if DYNAMIC_BOUNDARIES_ONLY
    else "BR_V3_SELF_CONTAINED_ALL_BOUNDARIES_STOCK_ADVISOR"
    if ENABLE_HYPOTHETICAL_STOCK_ADVISOR
    else "BR_V3_SELF_CONTAINED_ALL_BOUNDARIES"
)
...
SETUP_LABEL = "BREAKOUT_RESOLUTION"
RESOLUTION_ACCEPTED = "ACCEPTED"
RESOLUTION_FAILED = "FAILED"

logger = logging.getLogger("tests.test_breakout_resolution_report")
    ...
    for index, snapshot in enumerate(snapshots):
        ...
        if _within_entry_window(ts):
            for episode in episodes.values():
                ...
                accepted_key = (episode.episode_key, RESOLUTION_ACCEPTED, episode.breakout_side)
                if episode.last_seen_time == ts and accepted_key not in consumed_resolution_keys:
        ...
        # Terminal and consumed resolutions are already absent before arbitration.
        ...
        episode.state = (
            "ACCEPTED_SIGNAL_EMITTED"
            if selected.resolution == RESOLUTION_ACCEPTED
            else "FAILED_SIGNAL_EMITTED"
        )
...
def _print_summary(
    ...
    print("\nBREAKOUT_RESOLUTION REPORT SUMMARY")
    ...
    if rows:
        ...
        for row in rows[:PRINT_ROWS]:
            print(
                f"  {row.symbol:14s} {row.resolution:8s} {row.side:4s} "
...
def main() -> int:
    ...
    logger.info(
        "Starting BREAKOUT_RESOLUTION report | policy=%s dynamic_boundaries_only=%s stock_advisor=%s observation_only=True date=%s symbols=%s types=%s",
        POLICY_VERSION,
    ...
    for index, (symbol, snapshots) in enumerate(sorted(snapshots_by_symbol.items()), start=1):
        try:
            ...
            logger.exception("BREAKOUT_RESOLUTION evaluation failed for %s", symbol)
    ...
    logger.info(
        "Completed BREAKOUT_RESOLUTION report | policy=%s dynamic_boundaries_only=%s stock_advisor=%s observation_only=True output=%s signals=%d unique_episode_blockers=%s conflicts=%s",
        POLICY_VERSION,
        DYNAMIC_BOUNDARIES_ONLY,
        ENABLE_HYPOTHETICAL_STOCK_ADVISOR,
        output.resolve(),
    ...
    print("Share both files for comparison with ACCEPTED_BREAKOUT and FAILED_BREAKOUT outputs.")
    return 0
